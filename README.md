rule-app/
  ...
  ├── shared/   → common models and utils
  ├── .env.example
  └── package.json (workspace root)Rule App — Full Monorepo (Complete Full-code Scaffold)
...
export default async function handler(req,res){
  ...
  if(amount <= 0 || user.credits < amount) return res.status(400).jsonMONGODB_URI=mongodb://localhost:27017/ruleapp
JWT_SECRET=replace_with_a_strong_secret
NEXT_PUBLIC_BASE_URL=http://localhost:3000
SOCKET_IO_SECRET=replace_socket_secretnpm installdocker-compose up --buildnpm run dev# Rule-application-
New Google 
